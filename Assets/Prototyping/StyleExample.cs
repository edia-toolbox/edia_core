...
namespace StyleSheetExample {
    ...
    // INTERFACES:
    // - Name interfaces with adjective phrases. Ask is it 'Damageable'/'Interactable'/'DerivesLslTime'?
    // - Use the 'I' prefix.
...
#region ---- EVENT HOOKS
        ...
        private void SomeMethod() {
            ...
            // Stop listening
            // Remove the listener in memory from the eventmanager when it's not needed anymore
            ...
            // Some events require parameters
            // Use a new `eParam` to provide values to the event
            // eParams are defined in the `EventManager` and can be single value or lists of
            // int(s), float(s), boolean(s), string(s), vector3(s), transform(s), object(s)
            // and specific combinations: <string,bool>
            EventManager.TriggerEvent(Edia.Events.ControlPanel.EvShowMessageBox, new eParam("Message"));
        }
