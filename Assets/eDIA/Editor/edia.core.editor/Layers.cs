...
namespace eDIA.EditorUtils {
	...
	public class Layers {
		...
		/// <returns><c>true</c>, if layer was added, <c>false</c> otherwise.</returns>
		/// <param name="layerName">Layer name.</param>
		public static bool CreateLayer (int layerIndex, string layerName) {
			// Open tag manager
			SerializedObject tagManager = new SerializedObject (AssetDatabase.LoadAllAssetsAtPath ("ProjectSettings/TagManager.asset") [0]);
			// Layers Property
			SerializedProperty layersProp = tagManager.FindProperty ("layers");
			if (IsEmptyLayer (layersProp, layerIndex)) {
				SerializedProperty sp;
		...
		/// <param name="end">End.</param>
		/// <param name="value">Value.</param>
		private static bool IsEmptyLayer (SerializedProperty property, int index) {
			SerializedProperty t = property.GetArrayElementAtIndex (index);
			if (t.stringValue == "") {
				return true;
			} else return false;
		}
