        ...
        public void SetupTaskEnvironment() {
			...
			for (int s=0;s<StroopResponseFields.Count;s++) {
                StroopResponseFields[s].Init(
                    Experiment.Instance.CurrentBlock.settings.GetStringList("_colors")[s],
                    Experiment.Instance.CurrentBlock.settings.GetStringList("_words")[s],
                    Experiment.Instance.CurrentBlock.settings.GetString("target"),
					Experiment.Instance.CurrentBlock.settings.GetString("target") == "color" ?
                        Experiment.Instance.CurrentTrial.settings.GetString("color") :
                        Experiment.Instance.CurrentTrial.settings.GetString("word")
					);
            ...
            ColorUtility.TryParseHtmlString(Experiment.Instance.CurrentTrial.settings.GetString("color"), out col);
			StroopTextField.text  = Experiment.Instance.CurrentTrial.settings.GetString("word");
            StroopTextField.color = col;

            StroopTextField.fontSize = Experiment.Instance.CurrentBlock.settings.GetInt("font_size");
