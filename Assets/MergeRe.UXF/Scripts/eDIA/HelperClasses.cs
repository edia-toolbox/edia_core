...
namespace eDIA {
	...
	public class TaskConfig {
		...
		//		reordered.Add(Experiment.Instance.Blocks.Find(x => x.block_name == b.block_name));
		//	}

		//	Experiment.Instance.Blocks.Clear();
		//	Experiment.Instance.Blocks.AddRange(reordered);
			...
			foreach (SettingsTuple st in list)
				if (st.value.Contains(';')) { // it's a list!
					...
					for (int s = 0; s < stringlist.Count; s++) {
						string newstring = stringlist[s];
						if (newstring.StartsWith(" ")) {
							newstring = newstring.Substring(1);
						}
						stringlist[s] = newstring;
						//Debug.Log("adding:" + newstring);
					}
					tmp.Add(st.key, stringlist);
				}
				else {
					tmp.Add(st.key, st.value);  // normal string
												//Debug.Log("adding:" + st.value);
				}
