...
namespace Edia.Editor {
    ...
    [InitializeOnLoad]
    public class ExperimentSessionCheck : EditorWindow {
        ...
        static void Init() {
            if (!Application.isEditor) {
                Debug.LogWarning("Session Debugger can only be used in Editor mode");
                return;
            }

            var window = (ExperimentSessionCheck)EditorWindow.GetWindow(typeof(ExperimentSessionCheck));
            window.minSize      = new Vector2(300, 500);
            window.titleContent = new GUIContent("EDIA Session Debugger");
            window.Show();
        }

        static ExperimentSessionCheck() {
        ...
        static void FetchReferences() {
#if UNITY_6000
            experiment = Object.FindFirstObjectByType<Experiment>();
#else
            experiment = Object.FindObjectOfType<Experiment>();
#endif
            if (!experiment) {
                settingsDict = null;
                return;
            ...
            foreach (string key in experiment.settings.Keys)
                settingsDict.Add(key, experiment.settings.GetObject(key).ToString());
        ...
        public void OnGUI() {
            ...
            GUILayout.Label(
                "Press the button below after generating your blocks and trials to list all blocks and trials with their associated settings. This helps you make sure settings are being applied to the correct trials. ",
                labelStyle);
            ...
            if (settingsDict != null) {
                ...
                EditorGUILayout.HelpBox("Did not find an Experiment component in the scene! Press button above to try again.", MessageType.Error);
                return;
            }
            else if (!experiment.hasInitialised) {
                EditorGUILayout.HelpBox(
                    "Session has not yet started! This debugging tool can be used only when the session has started. Press button above to try again.",
                    MessageType.Warning);
                return;
            }
        ...
        static void ParseSettings() {
            EditorGUILayout.HelpBox(
                "Remember, Settings requests cascade upwards: That means accessing a setting in a trial will first look inside the trial, if it is then not found, will look inside the block, then the experiment session.",
                MessageType.Info);
